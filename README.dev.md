# `test_project` developer documentation
...
# install test_project as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `test_project`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d test_project.XXXXXX)
git clone git@github.com:smangham/test_project .
...
Visit
[https://test.pypi.org/project/test_project](https://test.pypi.org/project/test_project)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d test_project-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple test_project
```
...
Don't forget to also make a [release on GitHub](https://github.com/smangham/test_project/releases/new).GitHub-Zenodo integration will also trigger Zenodo into making a snapshot of your repository and sticking a DOI on it.